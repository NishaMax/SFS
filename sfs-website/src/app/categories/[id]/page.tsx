...
export default function CategoryItemsPage() {
  ...
  const { categories, categoryItems, loading: storeLoading } = useStore();
  ...
  const allItems = categoryItems.filter(item => item.categoryId === categoryId);
  ...
  if (!category) {
    return (
      <div className="min-h-screen flex flex-col justify-center items-center bg-gray-50">
        ...
        <Link href="/categories" className="text-green-600 mt-4 underline font-medium">← Back to Categories</Link>
      </div>
    );
  }
  ...
  const openDetail = (item: CategoryItem) => {
    ...
    // Set defaults for each variant label from the first matching SKU
    const firstSku = item.skus.find(s => s.brand === defaultBrand);
    setSelectedOptions(firstSku ? { ...firstSku.options } : {});
  };

  // Change brand and reset variant options
  ...
  // Change a variant option
  const handleOptionChange = (label: string, value: string) => {
    if (!selectedItem) return;
    const newOptions = { ...selectedOptions, [label]: value };
    // Check if this combo exists, if not, reset later labels
    const sku = findSKU(selectedItem, selectedBrand, newOptions);
    if (sku) {
      setSelectedOptions(newOptions);
    } else {
      // Find first valid combo with this option set
      const fallback = selectedItem.skus.find(s =>
        s.brand === selectedBrand && s.options[label] === value
      );
      if (fallback) {
    ...
    <main className="min-h-screen flex flex-col bg-gray-50/50">
      ...
      <div className="flex-1 pt-28 pb-16 px-4 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6">
          <div>
            <Link href="/categories" className="text-green-600 text-xs hover:text-green-800 transition-colors mb-2 inline-flex items-center gap-1.5 font-medium">
              <span>←</span>
              {language === 'en' ? 'All Categories' : language === 'si' ? 'සියලුම කාණ්ඩ' : 'அனைத்து வகைகள்'}
            </Link>
            <motion.h1 
              initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
              className="text-2xl md:text-4xl font-bold text-green-900"
            >
              {category.name[language]}
            </motion.h1>
            <p className="text-green-600 text-xs mt-1">{filteredItems.length} {language === 'en' ? 'items' : language === 'si' ? 'භාණ්ඩ' : 'பொருட்கள்'}</p>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="w-full sm:w-72">
            <div className="relative">
              ...
              <input
                ...
                placeholder={language === 'en' ? 'Search items or brands...' : language === 'si' ? 'භාණ්ඩ හෝ සන්නාම සොයන්න...' : 'பொருட்கள் அல்லது பிராண்ட்...'}
                className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500/40 focus:border-green-400 transition-all"
              />
        ...
        {filteredItems.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20 bg-white rounded-2xl border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500 font-medium text-sm">
              {search
                ? (language === 'en' ? 'No items match your search.' : language === 'si' ? 'ගැලපෙන භාණ්ඩ නැත.' : 'பொருத்தமான பொருட்கள் இல்லை.')
                : (language === 'en' ? 'More items coming soon!' : language === 'si' ? 'තවත් භාණ්ඩ ළඟදීම!' : 'மேலும் பொருட்கள் விரைவில்!')}
            </p>
          </motion.div>
        ) : (
            ...
            {filteredItems.map((item, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-lg hover:border-green-200 transition-all group cursor-pointer flex flex-col"
                >
                  {/* Image */}
                  <div className="w-full aspect-square relative bg-gray-50 overflow-hidden">
                    ...
                    {!anyInStock && (
                      <div className="absolute top-2 right-2 px-2 py-0.5 rounded-full text-[9px] font-bold text-white bg-red-500/90 backdrop-blur-sm">
                        {language === 'en' ? 'Out of Stock' : language === 'si' ? 'තොග අවසන්' : 'இல்லை'}
                      </div>
                    )}
                    {brands.length > 1 && (
                      <div className="absolute bottom-2 left-2 px-2 py-0.5 rounded-md text-[9px] font-bold text-green-800 bg-white/90 backdrop-blur-sm shadow-sm">
        ...
        {selectedItem && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 left-0 right-0 z-[201] bg-white rounded-t-3xl shadow-2xl max-h-[88vh] overflow-y-auto"
            >
              {/* Drag handle */}
              ...
              <div className="px-5 pb-8">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  {/* Image – changes with SKU */}
                  <div className="w-full md:w-64 aspect-square relative rounded-2xl overflow-hidden bg-gray-50 flex-shrink-0">
                    <Image
                    ...
                    {activeSKU && !activeSKU.inStock && (
                      <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                        <span className="bg-red-500 text-white px-4 py-1.5 rounded-full text-xs font-bold">
                          {language === 'en' ? 'Out of Stock' : language === 'si' ? 'තොග අවසන්' : 'கையிருப்பில் இல்லை'}
                        </span>
                      </div>
                    )}
                  </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {options.map(opt => {
                              ...
                              return (
                                <button
                                  key={opt}
                                  onClick={() => handleOptionChange(label, opt)}
